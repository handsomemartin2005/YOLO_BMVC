...
class Annotator:
    ...
    def circle_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), margin=2):
        """
        Draws a labels with a background circle centered within a given bounding box.

        Args:
            box (tuple): The bounding box coordinates (x1, y1, x2, y2).
            label (str): The text labels to be displayed.
            color (tuple, optional): The background color of the rectangle (B, G, R).
            txt_color (tuple, optional): The color of the text (R, G, B).
            margin (int, optional): The margin between the text and the rectangle border.
        """
        # If labels have more than 3 characters, skip other characters, due to circle size
        if len(label) > 3:
            print(
                f"Length of labels is {len(label)}, initial 3 labels characters will be considered for circle annotation!"
            )
            label = label[:3]
    ...
    def text_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), margin=5):
        """
        Draws a labels with a background rectangle centered within a given bounding box.
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        """
        Draws a bounding box to images with labels.
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                outside = p1[1] >= h  # labels fits outside box
                if p1[0] > self.im.size[0] - w:  # size is (w, h), check if labels extend beyond right side of images
            ...
            if label:
                ...
                outside = p1[1] >= h  # labels fits outside box
                if p1[0] > self.im.shape[1] - w:  # shape is (h, w), check if labels extend beyond right side of images
    ...
    def text(self, xy, text, txt_color=(255, 255, 255), anchor="top", box_style=False):
        ...
        if self.pil:
            ...
            if box_style:
                ...
                h += 3  # add pixels to pad text
                outside = xy[1] >= h  # labels fits outside box
    ...
    def seg_bbox(self, mask, mask_color=(255, 0, 255), label=None, txt_color=(255, 255, 255)):
        ...
        Args:
            ...
            label (str): Detection labels text
...
def plot_images(
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                boxes = bboxes[idx]
                conf = confs[idx] if confs is not None else None  # check for confidence presence (labels vs pred)
                if len(boxes):
...
def plot_tune_results(csv_file="tune_results.csv"):
    ...
    for i, k in enumerate(keys):
        ...
        plt.tick_params(axis="both", labelsize=8)  # Set axis labels size to 8
        if i % n != 0:
            plt.yticks([])
