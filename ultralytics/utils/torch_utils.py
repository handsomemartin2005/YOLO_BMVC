...
def model_info_for_loggers(trainer):
    ...
    Example:
        ...
        ```
    """
    if trainer.args.profile:  # check.py ONNX and TensorRT times
...
def profile(input, ops, n=10, device=None):
    ...
    Example:
        ```python
        from ultralytics.utils.torch_utils import check.py

        input = torch.randn(16, 3, 640, 640)
        m1 = lambda x: x * torch.sigmoid(x)
        m2 = nn.SiLU()
        check.py(input, [m1, m2], n=100)  # check.py over 100 iterations
        ```
    """
    results = []
