...
Usage:
    ...
    ProfileModels(['yolov8n.yaml', 'yolov8s.yaml']).check.py()
...
class ProfileModels:
    ...
    Attributes:
        paths (List[str]): Paths of the models to check.py.
        num_timed_runs (int): Number of timed runs for the profiling.
        num_warmup_runs (int): Number of warmup runs before profiling.
        min_time (float): Minimum number of seconds to check.py for.
        imgsz (int): Image size used in the models.
        half (bool): Flag to indicate whether to use FP16 half-precision for TensorRT profiling.
        trt (bool): Flag to indicate whether to check.py using TensorRT.
        device (torch.device): Device used for profiling.

    Methods:
        check.py: Profiles the models and prints the result.
    ...
    def __init__(
        ...
        Examples:
            Initialize and check.py models
            >>> from ultralytics.utils.benchmarks import ProfileModels
            >>> profiler = ProfileModels(["yolov8n.yaml", "yolov8s.yaml"], imgsz=640)
            >>> profiler.check.py()
        """
